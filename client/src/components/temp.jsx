...
const Home = () => {
  ...
  const [allBooks, setAllBooks] = useState([]); // Add this line if not already present
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAllBooks(data.allBooks); // Ensure your backend is sending allBooks data
      } catch (error) {
  ...
  const handleInputChange = (e) => {
    ...
    const matchedSuggestions = allBooks.filter((book) =>
          ...
          <div>
            ...
            <div className="mb-4 p-6 bg-gray-800 rounded-lg shadow-lg">
              ...
              {suggestions.length > 0 && (
                <ul className="bg-gray-700 text-white rounded-xl px-4 py-2 focus:outline-none">
